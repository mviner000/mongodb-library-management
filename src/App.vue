      ...
      if (currentActiveTabData) {
        ...
        if (currentActiveTabData.path !== newPath) {
          ...
          if (newPath.startsWith('/collection/')) {
            ...
            currentActiveTabData.title =
              pathParts.length > 0 ? pathParts[pathParts.length - 1] : 'Page'
            const matchedRoute = router
              .getRoutes()
              .find((r) => r.path === newPath || (r.path.includes(':') && isValidPath(newPath)))
    ...
    try {
      ...
      if (currentActiveTabData) {
        ...
        if (path.startsWith('/collection/')) {
          ...
          const matchedRoute = router
            .getRoutes()
            .find((r) => r.path === path || (r.path.includes(':') && isValidPath(path)))
          if (matchedRoute?.name === 'Collection') currentActiveTabData.type = 'collection'
          else if (matchedRoute?.name === 'Home') currentActiveTabData.type = 'home'
          else if (matchedRoute?.name === 'Hello') currentActiveTabData.type = 'hello'
  ...
  onMounted(async () => {
    ...
    if (authToken.value) {
      try {
        ...
        if (response.valid) {
          // Token is valid, fetch user data using the updated store
          await fetchUser()
          showAuthModal.value = false
        } else {
        ...
        <div :style="zoomStyle">
            ...
            <div class="p-4">
              ...
              <div class="flex flex-col items-center w-full gap-6">
                <MongoDBStatus
                  ...
                  @connection-status-changed="autoConnectMongoDB"
                />
                <MongoDBOperations
                  class="w-full max-w-3xl"
          ...
          <div v-else>
            ...
            <div v-else>
              ...
              <router-view
                :key="
                  activeTabId + '-' + (tabs.find((t) => t.id === activeTabId)?.reloadCount || 0)
                "
              />
            </div>
          </div>
