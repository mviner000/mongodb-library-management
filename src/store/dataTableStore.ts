 ...
 // Preload reference options needed for the current schema
  ...
  async function fetchReferenceOptions(refCollectionName: string, force = false) {
    ...
    try {
      ...
      if (!schemaSuccess) throw new Error(schemaError || `Failed to fetch schema for ${refCollectionName}`);
      ...
      const uniqueStringFields = Object.keys(properties).filter(field =>
         properties[field].bsonType === 'string' && properties[field].unique === true
      );
      ...
      const docsResponse = await fetch(`${API_BASE}/collections/${refCollectionName}/documents?limit=1000`); // Fetch all or limit as needed
      ...
      if (docsSuccess) {
        referenceOptions.value[refCollectionName] = docsData.map((doc: any) => ({
         ...
         console.log(`Workspaceed ${referenceOptions.value[refCollectionName].length} options for ${refCollectionName}`);
        if (referenceOptions.value[refCollectionName].length === 0) {
           toast({ title: 'No Reference Options', description: `No documents found in collection '${refCollectionName}'.`, variant: 'default' });
        }
      } else {
        throw new Error(docsError || `Failed to fetch documents for ${refCollectionName}`);
      }
    } catch (err: any) {
      toast({ title: 'Reference Error', description: `Failed to load options for ${refCollectionName}: ${err.message}`, variant: 'destructive' });
  ...
  function getDefaultValue(bsonType: string | string[]) {
    ...
    switch (type) {
      case 'string': return '';
      case 'int': case 'long': case 'double': case 'decimal': return 0;
      case 'bool': return false;
      case 'date': return new Date().toISOString(); // Store dates consistently, e.g., ISO string
      case 'objectId': return null; // Or handle differently if needed
      case 'object': return {};
      case 'array': return [];
      default: return null;
    }
  ...
  function startEditingCell(rowIndex: number, header: string, currentValue: any) {
    ...
    const bsonType = Array.isArray(schemaInfo.bsonType) ? schemaInfo.bsonType[0] : schemaInfo.bsonType;
  ...
  async function deleteDocument(docId: string) {
     ...
     try {
       ...
       if (errorMessage.value) { // Only clear if there was an error (success clears via fetch)
   ...
   function changeView(view: string) {
    ...
    async function saveColumnWidthsToBackend() {
         ...
         try {
             ...
             toast({ title: 'Save Error', description: `Could not save column widths: ${err.message}`, variant: 'destructive' });
